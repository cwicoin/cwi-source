...
#if defined(NDEBUG)
# error "cwi cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9cb1f7c4baa5c06cdcddc68a640553d477819058f8ec190f62c6ccbb2232e7d1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // cwi: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "cwi Signed Message:\n";
...
{
    // cwi: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // cwi: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // cwi: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1467578705, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c58cebf1428ee99c7ec152248a120844e921ad636e679c614bd43ae6c2b54df511241260e5c69e2c3361a964ffa79c312ddd75d819d74225e4cad867d2d1f481") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1467578705;
        ...
        assert(block.hashMerkleRoot == uint256("0xbdd9399bffefd214af469ec7aae5f65cc2ee86be63cb99710653b073ca76994b"));
...
unsigned char pchMessageStart[4] = { 0xd2, 0x0d, 0x14, 0xbd }; // cwi: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("cwiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("cwiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static cwiMiner(CWallet *pwallet)
{
    printf("cwiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cwi-miner");
    ...
    try { loop {
        ...
        printf("Running cwiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&cwiMiner, pwallet));
}
